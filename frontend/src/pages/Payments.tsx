  ...
  return (
    <>
      <div className="flex justify-between items-start">
        <div>
          <h1 className="font-montserrat text-3xl md:text-5xl font-bold text-on-background mb-2">Resumen de pagos ⛸️💰</h1>
            ...
            {invoices.map((invoice) => {
              ...
              return (
                <div
                  ...
                  className="flex items-center justify-between p-3 rounded-lg transition-colors border cursor-pointer hover:bg-bg-hover hover:border-border-subtle"
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div className="w-10 h-10 rounded-full bg-surface-container-high flex items-center justify-center text-primary-container font-inter text-sm border border-border-subtle">
                    ...
                    <div>
                      ...
                      <div className="text-xs text-on-surface-variant font-inter">
                        ...
                        {invoice.invoice_type === 'plan_renewal' && (
                          <span className="ml-2 px-1.5 py-0.5 rounded bg-amber-500/20 text-amber-500 text-[10px] font-semibold">
                            Renovación
                          </span>
                        )}
                      </div>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    {isAdmin && hasPendingProof && (
                      <div className="px-2 py-1 rounded-md bg-amber-500/20 text-amber-500 text-xs font-inter font-semibold whitespace-nowrap">
                        Revisar
                      </div>
                    )}
                    {isAdmin && !hasPendingProof && invoice.invoice_type === 'plan_renewal' && invoice.status === 'pending' && (
                      <button
                        ...
                        className="px-3 py-1.5 rounded-md bg-emerald-500/20 text-emerald-500 text-xs font-inter font-semibold hover:bg-emerald-500/30 transition-colors disabled:opacity-50 flex items-center gap-1"
                      >
                    ...
                    {isAdmin && !hasPendingProof && invoice.invoice_type !== 'plan_renewal' && (
                      <div className="flex gap-1 ml-2">
                        <button onClick={(e) => { e.stopPropagation(); setSelectedInvoice(invoice); setFormModalOpen(true); }} className="w-7 h-7 rounded-full hover:bg-bg-hover flex items-center justify-center text-on-surface-variant hover:text-primary transition-colors">
        ...
        <GlassCard>
          ...
          <div className="space-y-4">
            {transactions.map((txn) => (
              <div key={txn.id} className="p-3 rounded-lg bg-surface-container-high/50 border border-border-subtle">
